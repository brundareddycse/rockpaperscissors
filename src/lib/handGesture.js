...
export function detectGestureFromVideo(recognizer, videoEl, timestamp) {
  if (!recognizer || !videoEl || videoEl.readyState < 2) return null;
  ...
  try {
    ...
    if (result?.gestures?.length && result.gestures[0].length) {
      ...
      const move = GESTURE_MAP[top.categoryName] || null;
